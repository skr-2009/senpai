  ...
  candidates.forEach((student) => {
    ...
    li.innerHTML = `
      ...
      <div class="person-content">
        ...
        <p class="person-message">${escapeHtml(student.nickname)} / ${escapeHtml(student.grade)} / ${sentHeartIds.has(student.id) || getReceivedStudents().some((s) => s.id === student.id) ? "ハートでつながり中" : "プロフィールから相談OK"}</p>
      </div>
    `;
